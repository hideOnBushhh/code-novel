...
function activate(context) {
  ...
  let jumpChapter = vscode.commands.registerCommand(
    "extension.jumpChapter",
    () => {
      vscode.window
        .showQuickPick(chapterList.map((line) => line.value))
        .then((line) => {
          let value = chapterList.find((item) => item.value === line).page;
          console.log(value);
  ...
  context.subscriptions.push(jumpChapter);
...
function initNovelInfo() {
  ...
  chapterList = novelLines
    .map((line, index) => ({ value: line, page: Math.ceil(index / 100) }))
    .filter((line) =>
      line.value.match(
        /第[一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟0-9]{1,6}章/
      )
    );
}
// 初始化状态栏
function initStatusBar() {
  ...
  jumpBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left);
  jumpBar.text = "章节";
  jumpBar.command = "extension.jumpChapter";
}
// 获取当前页数据
function getPage() {
  const lines = novelLines.slice(
    (env["books"][env["currentBook"]] - 1) * 100,
    env["books"][env["currentBook"]] * 100
  );
  var template = tpl.format(...lines);
  fs.writeFileSync(novelPath, template);
  if (processBar) {
    processBar.text = `${env["currentBook"]}   ${
      env["books"][env["currentBook"]]
    } | ${totalPage}`;
  }
  scrollToTop();
